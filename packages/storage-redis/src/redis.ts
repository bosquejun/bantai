...
export function createRedisStorage<T extends z.ZodType>(
    ...
    const { prefix = "", lockTimeoutMs = 5_000, lockTTLMs = 10_000, lockRetryMs = 50 } = options;
    ...
    let acquireSha: Promise<string> | string = redisClient.script(
        "LOAD",
        ACQUIRE_SCRIPT
    ) as Promise<string>;
    let releaseSha: Promise<string> | string = redisClient.script(
        "LOAD",
        RELEASE_SCRIPT
    ) as Promise<string>;
