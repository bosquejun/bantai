import { ContextDefinition, ExtractContextShape, ExtractContextTools, RuleDefinition, auditEventSchema, defineContext, type AuditSink, type AuditTool } from "@bantai-dev/core";
import { z } from "zod";
...
type WithAuditTools<TContext extends ContextDefinition<z.ZodRawShape, Record<string, unknown>>> = 
  ExtractContextTools<TContext> & { audit: AuditTool<TContext, string, RuleDefinition<TContext, string>[]> };

export function withAudit<TContext extends ContextDefinition<z.ZodRawShape, Record<string, unknown>>>(context: TContext, options: WithAuditOptions): WithAuditContext<TContext> {
...
export type WithAuditContext<TContext extends ContextDefinition<z.ZodRawShape, Record<string, unknown>>> = ContextDefinition<
  WithAuditShape<TContext>,
  WithAuditTools<TContext>
>;
