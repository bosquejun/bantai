...
type StorageMap<
  ...
  TStorageNames extends string = string
> = Map<string, StorageAdapter<z.infer<TStorageDataSchema>>> & {
...
> => {
  ...
  const existingStorage = (context.tools as any).storage as StorageMap<TStorageDataSchema, any> | undefined;
  const storageMap = (existingStorage instanceof Map
    ? existingStorage
    : new Map<string, StorageAdapter<z.infer<TStorageDataSchema>>>()) as StorageMap<TStorageDataSchema, AllStorageNames<TContext, TStorageName>>;
