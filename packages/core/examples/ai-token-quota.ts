...
const TIER_QUOTAS: Record<string, { daily: number; hourly: number; monthly: number }> = {
    free: {
 ...
 */
const dailyQuotaCheck = defineRule(aiTokenQuotaContext, "daily-quota-check", async (input) => {
 ...
 */
const requestSizeCheck = defineRule(aiTokenQuotaContext, "request-size-check", async (input) => {
 ...
 */
const tierLimitCheck = defineRule(aiTokenQuotaContext, "tier-limit-check", async (input) => {
    const quota = TIER_QUOTAS[input.tier];
    if (!quota) {
        return deny({
            reason: `Invalid tier: ${input.tier}`,
 ...
 */
const degradingQuality = defineRule(aiTokenQuotaContext, "degrading-quality", async (input) => {
    const quota = TIER_QUOTAS[input.tier];
    if (!quota) {
        return allow({
