...
const COUNTRY_NAME_FORMATS: Record<string, {
  allowNumbers: boolean;
  allowSpecialChars: boolean;
  allowedSpecialChars?: string[];
}> = {
  US: { allowNumbers: false, allowSpecialChars: true, allowedSpecialChars: ["'", '-'] },
  UK: { allowNumbers: false, allowSpecialChars: true, allowedSpecialChars: ["'", '-'] },
  ...
  BR: { allowNumbers: false, allowSpecialChars: true, allowedSpecialChars: ["'", '-'] },
  MX: { allowNumbers: false, allowSpecialChars: true, allowedSpecialChars: ["'", '-'] },
};

// Default format rules
const DEFAULT_FORMAT = { allowNumbers: false, allowSpecialChars: true, allowedSpecialChars: ["'", '-'] };
  ...
  async (input) => {
    ...
    if (input.firstName) {
      if (input.firstName.length < MIN_NAME_LENGTH) {
        violations.push(`First name must be at least ${MIN_NAME_LENGTH} character(s) long`);
      }
      if (input.firstName.length > MAX_NAME_LENGTH) {
        violations.push(`First name must be no more than ${MAX_NAME_LENGTH} characters long`);
      }
    }
    
    if (input.lastName) {
      if (input.lastName.length < MIN_NAME_LENGTH) {
        violations.push(`Last name must be at least ${MIN_NAME_LENGTH} character(s) long`);
      }
      if (input.lastName.length > MAX_NAME_LENGTH) {
        violations.push(`Last name must be no more than ${MAX_NAME_LENGTH} characters long`);
      }
    }
    
    if (input.fullName) {
      if (input.fullName.length < MIN_FULL_NAME_LENGTH) {
        violations.push(`Full name must be at least ${MIN_FULL_NAME_LENGTH} characters long`);
      }
      if (input.fullName.length > MAX_FULL_NAME_LENGTH) {
        violations.push(`Full name must be no more than ${MAX_FULL_NAME_LENGTH} characters long`);
      }
    }
...
const nameFormatCheck = defineRule(
  ...
  async (input) => {
    ...
    if (input.firstName && !basicFormatRegex.test(input.firstName)) {
      violations.push('First name contains invalid characters. Only letters, spaces, hyphens, and apostrophes are allowed');
    }
    
    if (input.lastName && !basicFormatRegex.test(input.lastName)) {
      violations.push('Last name contains invalid characters. Only letters, spaces, hyphens, and apostrophes are allowed');
    }
    
    if (input.fullName && !basicFormatRegex.test(input.fullName)) {
      violations.push('Full name contains invalid characters. Only letters, spaces, hyphens, and apostrophes are allowed');
    }
    
    // Check for consecutive special characters
    const consecutiveSpecialRegex = /['-]{2,}/;
    if (input.firstName && consecutiveSpecialRegex.test(input.firstName)) {
      violations.push('First name contains consecutive special characters');
    }
    if (input.lastName && consecutiveSpecialRegex.test(input.lastName)) {
      violations.push('Last name contains consecutive special characters');
    }
    if (input.fullName && consecutiveSpecialRegex.test(input.fullName)) {
      violations.push('Full name contains consecutive special characters');
    }
...
const countrySpecificFormat = defineRule(
  ...
  async (input) => {
    ...
    const formatRules = COUNTRY_NAME_FORMATS[input.country] || DEFAULT_FORMAT;
    const violations: string[] = [];
    
    const checkName = (name: string, fieldName: string) => {
      if (formatRules.allowNumbers && /[0-9]/.test(name)) {
        violations.push(`${fieldName} contains numbers, which are not allowed for country ${input.country}`);
      }
      
      if (!formatRules.allowSpecialChars) {
        const specialCharRegex = /[^a-zA-Z\s]/;
        if (specialCharRegex.test(name)) {
          violations.push(`${fieldName} contains special characters, which are not allowed for country ${input.country}`);
        }
        ...
        const disallowedSpecialRegex = new RegExp(`[^a-zA-Z\\s${allowedChars.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}]`);
        if (disallowedSpecialRegex.test(name)) {
          violations.push(`${fieldName} contains special characters not allowed for country ${input.country}. Allowed: ${formatRules.allowedSpecialChars.join(', ')}`);
        }
      }
    };
  ...
  async (input) => {
    ...
    const checkName = (name: string, fieldName: string) => {
      ...
      if (BLOCKED_WORDS.has(nameLower)) {
        violations.push(`${fieldName} contains a blocked word: "${name}"`);
        return;
      }
      ...
      for (const blocked of BLOCKED_WORDS) {
        if (nameLower.includes(blocked)) {
          violations.push(`${fieldName} contains a blocked word: "${blocked}"`);
        }
      }
    };
  ...
  async (input) => {
    ...
    if (input.firstName) {
      if (startsWithSpecial.test(input.firstName)) {
        violations.push('First name cannot start with a special character');
      }
      if (endsWithSpecial.test(input.firstName)) {
        violations.push('First name cannot end with a special character');
      }
    }
    
    if (input.lastName) {
      if (startsWithSpecial.test(input.lastName)) {
        violations.push('Last name cannot start with a special character');
      }
      if (endsWithSpecial.test(input.lastName)) {
        violations.push('Last name cannot end with a special character');
      }
    }
    
    if (input.fullName) {
      if (startsWithSpecial.test(input.fullName)) {
        violations.push('Full name cannot start with a special character');
      }
      if (endsWithSpecial.test(input.fullName)) {
        violations.push('Full name cannot end with a special character');
      }
    }
...
const nameValidationPolicy = definePolicy(
  ...
  [nameLengthCheck, nameFormatCheck, countrySpecificFormat, profanityCheck, specialCharacterCheck],
  {
