...
const COLLECTION_POLICIES: Record<string, {
...
// Marketplace restrictions
const MARKETPLACE_RESTRICTIONS: Record<string, {
 ...
 */
const transferEligibility = defineRule(
  cryptoNftContext,
  'transfer-eligibility',
  async (input) => {
 ...
 */
const gasPriceCheck = defineRule(
  cryptoNftContext,
  'gas-price-check',
  async (input) => {
    if (!input.gasPrice) {
    ...
    if (input.marketplace) {
      ...
      if (marketplaceRestrictions?.minGasPrice && input.gasPrice < marketplaceRestrictions.minGasPrice) {
        return deny({
 ...
 */
const collectionPolicy = defineRule(
  cryptoNftContext,
  'collection-policy',
  async (input) => {
    if (!input.collection) {
 ...
 */
const networkWhitelist = defineRule(
  cryptoNftContext,
  'network-whitelist',
  async (input) => {
...
const cryptoNftPolicy = definePolicy(
  cryptoNftContext,
  'crypto-nft-policy',
  [ownershipVerification, transferEligibility, gasPriceCheck, collectionPolicy, royaltyCheck, marketplaceRestrictions, networkWhitelist],
  {
