...
const quantityLimitCheck = defineRule(
	...
	async (input) => {
		...
		if (
			input.product.maxQuantity &&
			input.quantity > input.product.maxQuantity
		) {
			return deny({
...
const shippingAddressCheck = defineRule(
	...
	async (input) => {
		...
		if (
			...
			input.product.restrictedRegions.includes(
...
const cartTotalCheck = defineRule(
	...
	async (input) => {
		...
		const cartTotal = input.cart.reduce(
...
const cartInventoryCheck = defineRule(
	...
	async (input) => {
		...
		const totalItems = input.cart.reduce(
