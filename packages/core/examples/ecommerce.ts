...
const ecommerceSchema = z.object({
    ...
    paymentMethod: z
        .enum(["credit-card", "debit-card", "paypal", "bank-transfer", "crypto"])
        .optional(),
    shippingAddress: z
        .object({
 ...
 */
const inventoryCheck = defineRule(ecommerceContext, "inventory-check", async (input) => {
    if (!input.product) {
        return allow({
 ...
 */
const quantityLimitCheck = defineRule(ecommerceContext, "quantity-limit-check", async (input) => {
    if (!input.product) {
        return allow({
 ...
 */
const paymentMethodCheck = defineRule(ecommerceContext, "payment-method-check", async (input) => {
...
const shippingAddressCheck = defineRule(
    ...
    async (input) => {
        ...
        if (
            input.product.restrictedRegions &&
            input.product.restrictedRegions.includes(input.shippingAddress.country)
        ) {
            return deny({
        ...
        for (const [restrictingCountry, restrictedCountries] of Object.entries(
 ...
 */
const cartTotalCheck = defineRule(ecommerceContext, "cart-total-check", async (input) => {
 ...
 */
const cartInventoryCheck = defineRule(ecommerceContext, "cart-inventory-check", async (input) => {
    if (!input.cart || input.cart.length === 0) {
        return allow({
...
async function runExamples() {
    ...
    const result2 = await evaluatePolicy(ecommercePolicy, {
        userId: "user123",
        cart: [{ productId: "prod1", quantity: 2, price: 25.99 }],
        product: {
            id: "prod1",
    ...
    const result6 = await evaluatePolicy(ecommercePolicy, {
        userId: "user123",
        cart: [{ productId: "prod1", quantity: 10, price: 25.99 }],
        product: {
            id: "prod1",
            inventory: 20,
