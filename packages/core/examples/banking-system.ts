...
const bankingSchema = z.object({
    userId: z.string(),
    accountId: z.string(),
    transactionType: z.enum(["transfer", "withdrawal", "deposit", "payment", "wire-transfer"]),
 ...
 */
const balanceCheck = defineRule(bankingContext, "balance-check", async (input) => {
    ...
    if (balanceAfterTransaction < MIN_BALANCE && input.transactionType !== "withdrawal") {
        return deny({
 ...
 */
const dailyLimitCheck = defineRule(bankingContext, "daily-limit-check", async (input) => {
 ...
 */
const transactionTypeCheck = defineRule(bankingContext, "transaction-type-check", async (input) => {
 ...
 */
const kycVerification = defineRule(bankingContext, "kyc-verification", async (input) => {
    ...
    if (input.amount >= ENHANCED_KYC_THRESHOLD && input.kycStatus !== "verified") {
        return deny({
 ...
 */
const fraudDetection = defineRule(bankingContext, "fraud-detection", async (input) => {
 ...
 */
const recipientValidation = defineRule(bankingContext, "recipient-validation", async (input) => {
    ...
    if (input.transactionType === "transfer" || input.transactionType === "wire-transfer") {
 ...
 */
const velocityCheck = defineRule(bankingContext, "velocity-check", async (input) => {
