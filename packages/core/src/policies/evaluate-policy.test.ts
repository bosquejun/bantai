...
describe("evaluatePolicy", () => {
	describe("preemptive strategy", () => {
		it("should return allow when all rules pass", async () => {
			...
			const isAdmin = defineRule(context, "is-admin", async (input) => {
				return input.role === "admin"
					? allow({ reason: "User is an admin" })
					: deny();
			});

			const policy = definePolicy(context, "test-policy", [
				isAdult,
				isAdmin,
			]);
		...
		it("should return deny on first violation with preemptive strategy", async () => {
			...
			const isAdmin = defineRule(context, "is-admin", async (input) => {
				return input.role === "admin"
					? allow()
					: deny({ reason: "User is not an admin" });
			});
	...
	describe("type inference", () => {
		it("infers input from policy context", () => {
			const schema = z.object({
				age: z.number(),
				role: z.enum(["admin", "user"]),
			});
			...
			const rule = defineRule(context, "is-adult", async (input) => {
				// `input` should be inferred from `schema`
				const _check: Expect<
					Equal<typeof input, z.infer<typeof schema>>
				> = true;
				return input.age >= 18 ? allow() : deny();
			});

			const policy = definePolicy(context, "typed-policy", [rule]);

			// correct usage compiles
			void evaluatePolicy(policy, { age: 30, role: "admin" });

			// The following calls are wrapped in `if (false)` so they are never
			// executed at runtime, but still type-checked by TypeScript.
			if (false as boolean) {
				// @ts-expect-error - missing required field `role`
				void evaluatePolicy(policy, { age: 30 });

				// @ts-expect-error - invalid literal for `role`
				void evaluatePolicy(policy, { age: 30, role: "superadmin" });
			}
		});

		it("supports contexts with tools and audit typing", () => {
			const schema = z.object({
				value: z.number(),
				audit: z
					.object({
			...
			const context = defineContext(schema, {
				tools: {
					audit: {
						// types are validated at compile-time via `AuditTool` in implementation;
						// keep these parameters as `unknown` to avoid implicit `any`.
						createAuditEvent: (
							_policy: unknown,
							_evaluationId: unknown
						) => ({
							emit: (_event: unknown) => `event:dummy`,
						}),
					},
				},
			});

			const rule = defineRule(context, "always-allow", async () =>
				allow()
			);

			const policy = definePolicy(context, "policy-with-audit", [rule]);

			// should allow `audit` to be omitted, optional, or provided with trace
			void evaluatePolicy(policy, { value: 1 });
			void evaluatePolicy(policy, { value: 1, audit: {} });
			void evaluatePolicy(policy, {
				value: 1,
				audit: { trace: "trace-id" },
			});
		});
	});
		...
		it("should collect all violations with exhaustive strategy", async () => {
			...
			const rule1 = defineRule(context, "rule-1", async (input) => {
				return input.value > 0
					? allow()
					: deny({ reason: "Value must be positive" });
			});

			const rule2 = defineRule(context, "rule-2", async (input) => {
				return input.value < 100
					? allow()
					: deny({ reason: "Value must be less than 100" });
			});

			const rule3 = defineRule(context, "rule-3", async (input) => {
				return input.value % 2 === 0
					? allow()
					: deny({ reason: "Value must be even" });
			});
			...
			expect(result.decision).toBe("deny");
			expect(result.violatedRules).toHaveLength(2);
			expect(result.violatedRules.map((r) => r.name)).toEqual([
				"rule-1",
				"rule-3",
			]);
		});
	...
	describe("strategy options", () => {
		it("should use policy default strategy when not specified", async () => {
			...
			const rule = defineRule(context, "test-rule", async (_input) =>
				deny()
			);
