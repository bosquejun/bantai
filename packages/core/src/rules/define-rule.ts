...
export type ExtractTools<T> =
	T extends ContextDefinition<infer _, infer TTools> ? TTools : {};
...
>(
	...
	const rule = ruleSchema.parse({
		...
		evaluate: evalFnSchema.implementAsync(
			wrappedEvaluate as unknown as Parameters<
				typeof evalFnSchema.implementAsync
			>[0]
		),
		hooks: {
