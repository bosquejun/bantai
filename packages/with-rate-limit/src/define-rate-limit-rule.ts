 ...
 */
type RuleEvaluateFnAsync<T extends z.ZodRawShape, TTools extends Record<string, unknown> = {}> = (
 ...
 */
type RuleHookFnAsync<T extends z.ZodRawShape, TTools extends Record<string, unknown> = {}> = (
    result: RuleResult,
...
>(
    ...
    if (!context.tools.rateLimit || !context.tools.storage) {
        throw new Error(
            "Rate limit and storage are required. Please use the withRateLimit function to extend the context with rate limit capabilities."
        );
    }
    return defineRule(
        context,
        ...
        async (input, ctx) => {
            ...
            return await evaluate(
                { ...typedInput, currentLimit: rateLimitResult } as z.infer<
                    z.ZodObject<ExtractContextShape<TContext>>
                > & { currentLimit: RateLimitCheckResult },
                ctx
            );
        },
        {
            onAllow: async (result, input, ctx) => {
                ...
                if (options?.onAllow) {
                    await options.onAllow(
                        result,
                        typedInput as z.infer<z.ZodObject<ExtractContextShape<TContext>>>,
                        ctx
                    );
                }
            },
