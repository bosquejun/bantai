...
export const Capabilities: React.FC = () => {
  return (
    <section id="features" className="py-40 bg-background relative border-b border-input">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        <div className="mb-32">
            <span className="text-foreground/20 font-mono text-[10px] font-bold uppercase tracking-[0.5em] mb-4 block">CORE_PILLARS</span>
            <h3 className="text-5xl lg:text-7xl font-bold text-foreground tracking-tighter uppercase leading-none font-heading">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-24">
          {PILLARS.map((item, i) => (
            <div key={i} className="group">
              <div className="w-12 h-[2px] bg-foreground mb-8 group-hover:w-full transition-all duration-700 opacity-20 group-hover:opacity-100"></div>
              <h4 className="text-2xl font-black mb-6 uppercase tracking-tight text-foreground">{item.title}</h4>
              <p className="text-lg text-foreground/50 font-medium uppercase tracking-tight leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
