...
### [Audit Logging](/docs/extensions/with-audit)

Add comprehensive audit logging capabilities to your Bantai contexts:

- **Automatic Event Tracking**: Automatically tracks policy evaluations and rule decisions
- **Multiple Event Types**: Policy start/end, rule start/decision/end, and custom events
- **Multiple Sinks**: Send events to multiple destinations (console, database, logging services)
- **Trace Correlation**: Support for trace IDs and request IDs for distributed tracing

Perfect for compliance, debugging, and observability.
...
const contextWithAudit = withAudit(baseContext, {
  sinks: [
    ...
    async (event) => await db.auditEvents.insert(event),
  ],
});
```
...
const rateLimitContext = withRateLimit(storageContext, {
  storage: redisStorage,
  generateKey: (input) => `api:${input.userId}:${input.endpoint}`,
});
const enhancedContext = withAudit(rateLimitContext, {
  sinks: [
    (event) => console.log('Audit:', event),
    async (event) => await db.auditEvents.insert(event),
  ],
...
const rule = defineRule(enhancedContext, 'api-rule', async (input, { tools }) => {
  // Access storage, rateLimit, and audit tools
  ...
  // Audit events are automatically emitted during policy evaluation
  // ...
});
```
...
- **Audit Logging**: Track policy evaluations, rule decisions, and custom events for compliance and observability
...
# Audit logging
npm install @bantai-dev/with-audit
...
- **[Audit Extension](/docs/extensions/with-audit)** - Learn about audit logging
