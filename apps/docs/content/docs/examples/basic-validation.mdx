...
const countryAgeRule = defineRule(
  ...
  async (input) => {
    if (!input.country) {
      return skip({ 
        reason: 'Country not provided, skipping country-specific check',
        meta: { skippedReason: 'missing_country' }
      });
    }
    ...
    if (input.age >= requiredAge) {
      return allow({ 
        reason: `User meets ${input.country} age requirement`,
        meta: { country: input.country, requiredAge, providedAge: input.age }
      });
    }
    return deny({ 
      reason: `User must be ${requiredAge} or older in ${input.country}`,
      meta: { country: input.country, requiredAge, providedAge: input.age }
    });
  }
);
