...
import { getPackageData } from "../load-playground-deps.ts";
import { getPackageVersions } from "../packageMetadata.ts";

export const ZOD_PACKAGE_NAME = "zod";
export const ZOD_PACKAGE_VERSION = "4.3.5";

export const ZOD_GLOBAL_DECLARATION = `
declare global {
    const z: typeof import("${ZOD_PACKAGE_NAME}").z;
}
...
export async function getVersions(
    ...
    const packageVersions = await getPackageVersions({ packageName: ZOD_PACKAGE_NAME, tag });
...
export function validateSchema(schema: string): SchemaValidation {
    try {
        ...
        const data = new Function("z", transpiledCode)(
            getPackageData(ZOD_PACKAGE_NAME, ZOD_PACKAGE_VERSION)
        ) as ZodSchema;
...
export default {
    zod: {
        name: ZOD_PACKAGE_NAME,
        version: ZOD_PACKAGE_VERSION,
        globalDeclaration: ZOD_GLOBAL_DECLARATION,
    },
};
